...
int find_minimum( const vector<vector<int>>& v) {								// no issues
...
int find_maximum( const vector<vector<int>>& v) {								// no issues
...
int calculate_gray( int elevation, int min_elevation, int max_elevation ) {		// no issues
...
int colorPath(const vector<vector<int>>& heightMap, vector<vector<int>>& r, vector<vector<int>>& g, vector<vector<int>>& b, int color_r, int color_g, int color_b, int start_row) {											// issue: path directions are rotated clockwise 90 degrees
	int total_dist = 0;
	int row = start_row;
	int tempSize = 480;
	// loop until we reach the last column
	for(int col = 0; col < tempSize /* heightMap.size() */; ++col){
		// color the cell we are at with the provided RGB values
		r[row][col] = color_r;													// fixed output direction by switching rows and columns,
		g[row][col] = color_g;													// but paths are still calculated from rotated map.
		b[row][col] = color_b;
		
		// if we are on the last column
		if(col > tempSize /* heightMap.size() */ - 1){
			break;
		}
