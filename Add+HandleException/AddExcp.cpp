                    ...
                    double& x1, double& x2)
{
    ...
    if(disc < 0){
    	throw runtime_error("cannot calculate roots when the discriminant is negative...");
    }
...
try{
            ...
            outputRoots(x1, x2);
}catch(runtime_error &e){cout << "No real roots." << endl;}
        }
        cout << endl << "Input 'y' or 'Y' to do another. Input anything else to exit." << endl;
        cin >> cont;
        cont = tolower(cont);
    }
} catch (std::runtime_error &e) { cerr << "ERROR: " << e.what() << endl; return 1; }
}
